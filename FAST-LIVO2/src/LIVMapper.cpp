...
LIVMapper::~LIVMapper() 
{
  // Save PCD on exit to ensure data is not lost if process is killed
  ROS_INFO("[Shutdown] Saving point cloud data before exit...");
  savePCD();
  ROS_INFO("[Shutdown] Cleanup completed.");
}
...
void LIVMapper::initializeSubscribersAndPublishers(ros::NodeHandle &nh, image_transport::ImageTransport &it) 
{
  ...
  const int imu_queue_size = 100;
  const int image_queue_size = 5;
  bool subscribe_compressed_fallback = false;

  nh.param<bool>("common/subscribe_compressed_fallback", subscribe_compressed_fallback, false);

  sub_pcl = p_pre->lidar_type == AVIA ? 
            nh.subscribe(lid_topic, lidar_queue_size, &LIVMapper::livox_pcl_cbk, this): 
            nh.subscribe(lid_topic, lidar_queue_size, &LIVMapper::standard_pcl_cbk, this);
  sub_imu = nh.subscribe(imu_topic, imu_queue_size, &LIVMapper::imu_cbk, this);
  
  // Try to subscribe to raw image first, if not available try compressed
  sub_img = nh.subscribe(img_topic, image_queue_size, &LIVMapper::img_cbk, this);
  ROS_INFO("Subscribed to raw image topic: %s", img_topic.c_str());
  
  if (subscribe_compressed_fallback)
  {
    // Optional compressed fallback. Keep disabled by default to avoid duplicate image streams.
    string compressed_topic = img_topic + "/compressed";
    sub_img_compressed = nh.subscribe(compressed_topic, image_queue_size, &LIVMapper::img_compressed_cbk, this);
    ROS_INFO("Subscribed to compressed image topic: %s", compressed_topic.c_str());
  }
  else
  {
    ROS_INFO("Compressed fallback subscription disabled");
  }
...
void LIVMapper::handleVIO() 
{
  ...
  // Build visual sub-map for visualization
  visual_sub_map->clear();
  for (int i = 0; i < vio_manager->visual_submap->voxel_points.size(); i++)
  {
    VisualPoint *pt = vio_manager->visual_submap->voxel_points[i];
    if (pt == nullptr || pt->pos_.norm() == 0) continue;
  ...
  // Also add points from voxel map
  for (int i = 0; i < vio_manager->visual_submap->add_from_voxel_map.size(); i++)
  {
    PointType temp_map;
    temp_map.x = vio_manager->visual_submap->add_from_voxel_map[i].point_w[0];
    temp_map.y = vio_manager->visual_submap->add_from_voxel_map[i].point_w[1];
    temp_map.z = vio_manager->visual_submap->add_from_voxel_map[i].point_w[2];
    temp_map.intensity = 0.;
    visual_sub_map->push_back(temp_map);
  }
...
void LIVMapper::savePCD() 
{
  if (pcd_save_en && (pcl_wait_save->points.size() > 0 || pcl_wait_save_intensity->points.size() > 0) && pcd_save_interval < 0) 
  {
    ...
    if (img_en)
    {
      ...
      size_t cleared_points = pcl_wait_save->size();
      PointCloudXYZRGB().swap(*pcl_wait_save);
      ROS_INFO("[Memory] Cleared pcl_wait_save, freed %zu points", cleared_points);
    }
    else
    {      
      ...
      ROS_INFO("[Memory] Cleared pcl_wait_save_intensity");
    }
  }
}
...
void LIVMapper::run() 
{
  ...
  while (ros::ok()) 
  {
    ...
    // Memory monitoring: check every 100 frames
    ...
    if (memory_check_counter >= 100)
    {
      ...
      ROS_INFO_THROTTLE(5.0, "[Memory Status] img_buffer: %zu, lidar_buffer: %zu, imu_buffer: %zu",
                        img_buffer.size(), lid_raw_data_buffer.size(), imu_buffer.size());
      // Memory management: monitor and limit buffer sizes
      const size_t MAX_LIDAR_BUFFER = 8;   // Optimized for 12GB RAM systems (was 3)
      const size_t MAX_IMU_BUFFER = 120;   // Optimized for 12GB RAM systems (was 50)
      
      if (lid_raw_data_buffer.size() > MAX_LIDAR_BUFFER)
      {
        ROS_WARN("[Memory] LiDAR buffer too large (%zu), clearing oldest", lid_raw_data_buffer.size());
      ...
      if (imu_buffer.size() > MAX_IMU_BUFFER)
      {
        ROS_WARN("[Memory] IMU buffer too large (%zu), clearing oldest", imu_buffer.size());
        while (imu_buffer.size() > 10)
        {
...
void LIVMapper::standard_pcl_cbk(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  {
    double time_diff = last_timestamp_lidar - cur_head_time;
    if (time_diff > 1.0) {
      ROS_WARN("LiDAR timestamp jump detected: %.3f secs, clearing buffer\n", time_diff);
...
void LIVMapper::imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in)
{
  ...
  // 容忍大的时间跳变（M2DGR数据集常见问题）
  if (last_timestamp_imu > 0.0 && timestamp < last_timestamp_imu)
  {
    double time_diff = last_timestamp_imu - timestamp;
    if (time_diff > 1.0) {
      // 时间戳大幅回退，清空buffer重新开始
      ROS_WARN("IMU timestamp jump detected: %.3f secs, clearing buffer\n", time_diff);
      std::deque<sensor_msgs::Imu::ConstPtr> empty_queue;
      std::swap(imu_buffer, empty_queue);
...
cv::Mat LIVMapper::getImageFromMsg(const sensor_msgs::ImageConstPtr &img_msg)
{
  ...
  // Check if the image is in Bayer format and convert accordingly
  if (img_msg->encoding == "bayer_rggb8" || img_msg->encoding == "bayer_bggr8" || 
      img_msg->encoding == "bayer_gbrg8" || img_msg->encoding == "bayer_grbg8")
  {
    // Demosaic Bayer to RGB - use shared_ptr to avoid unnecessary copies
    cv_bridge::CvImageConstPtr cv_ptr = cv_bridge::toCvShare(img_msg, img_msg->encoding);
    
    // Then demosaic to RGB
    int demosaic_code;
    if (img_msg->encoding == "bayer_rggb8")
      demosaic_code = cv::COLOR_BayerBG2RGB;  // OpenCV uses opposite naming
    else if (img_msg->encoding == "bayer_bggr8")
      demosaic_code = cv::COLOR_BayerRG2RGB;
    else if (img_msg->encoding == "bayer_gbrg8")
      demosaic_code = cv::COLOR_BayerGR2RGB;
    else if (img_msg->encoding == "bayer_grbg8")
      demosaic_code = cv::COLOR_BayerGB2RGB;
    
    cv::cvtColor(cv_ptr->image, img, demosaic_code);
    ROS_INFO("[Bayer Convert] Input encoding: %s, Output channels: %d, Size: %dx%d", 
             img_msg->encoding.c_str(), img.channels(), img.cols, img.rows);
  }
  else if (img_msg->encoding == "mono8" || img_msg->encoding == "8UC1")
  {
    // For grayscale images, convert to BGR for consistent processing
    cv_bridge::CvImageConstPtr cv_ptr = cv_bridge::toCvShare(img_msg, "mono8");
    cv::cvtColor(cv_ptr->image, img, cv::COLOR_GRAY2BGR);
    ROS_INFO("[Mono Convert] Output channels: %d", img.channels());
  }
  else
  {
    // For already RGB/BGR images - use share instead of copy to reduce memory
    cv_bridge::CvImageConstPtr cv_ptr = cv_bridge::toCvShare(img_msg, "bgr8");
    img = cv_ptr->image.clone(); // Clone to ensure we have our own copy
    ROS_INFO("[Direct Convert] Input encoding: %s, Output channels: %d", 
             img_msg->encoding.c_str(), img.channels());
  }
...
void LIVMapper::img_cbk(const sensor_msgs::ImageConstPtr &msg_in)
{
  ...
  // Memory management: limit image buffer size to prevent OOM
  const size_t MAX_IMG_BUFFER_SIZE = 12;  // Optimized for 12GB RAM systems
  if (img_buffer.size() >= MAX_IMG_BUFFER_SIZE)
  {
    ROS_WARN_THROTTLE(1.0, "[Memory] Image buffer full (%zu), clearing oldest images", img_buffer.size());
    // Clear oldest images to free memory
    while (img_buffer.size() > 2)
    {
      img_buffer.pop_front();
      img_time_buffer.pop_front();
    }
    // Force memory release by releasing the front element
    if (!img_buffer.empty())
    {
...
void LIVMapper::img_compressed_cbk(const sensor_msgs::CompressedImageConstPtr &msg_in)
{
  if (!img_en) return;
  
  try {
    // Decode compressed image - try multiple formats
    std::string encoding = "bgr8";
    
    // Check the format string to determine actual encoding
    if (msg_in->format.find("rgb8") != std::string::npos) {
      encoding = "rgb8";
      ROS_INFO_ONCE("[Compressed] Detected RGB8 format");
    } else if (msg_in->format.find("bgr8") != std::string::npos) {
      encoding = "bgr8";
      ROS_INFO_ONCE("[Compressed] Detected BGR8 format");
    }
    
    cv_bridge::CvImagePtr cv_ptr = cv_bridge::toCvCopy(msg_in, encoding);
    ...
    // Create a lightweight Image message instead of full copy
    ...
    msg->step = cv_ptr->image.step;
    msg->data.assign(cv_ptr->image.datastart, cv_ptr->image.dataend);
    
    ROS_INFO_THROTTLE(1.0, "[Compressed Decode] Size: %dx%d, Format: %s -> BGR8", 
                      msg->width, msg->height, msg_in->format.c_str());
    
    // Call the regular image callback
    img_cbk(msg);
    
    // Explicitly release to free memory
    cv_ptr.reset();
    msg.reset();
  }
  catch (cv_bridge::Exception& e) {
    ROS_ERROR("cv_bridge exception for compressed image: %s", e.what());
    return;
  }
}
...
void LIVMapper::publish_img_rgb(const image_transport::Publisher &pubImage, VIOManagerPtr vio_manager)
{
  ...
  if (img_rgb.empty()) {
    ROS_WARN_THROTTLE(1.0, "[Publish RGB] img_cp is empty!");
    return;
  }
  
  ROS_INFO_THROTTLE(2.0, "[Publish RGB] Channels: %d, Size: %dx%d, Type: %d", 
                    img_rgb.channels(), img_rgb.cols, img_rgb.rows, img_rgb.type());
...
void LIVMapper::publish_frame_world(const ros::Publisher &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  // For LIO/LO mode, publish directly without RGB
  if (slam_mode_ == ONLY_LIO || slam_mode_ == ONLY_LO || LidarMeasures.lio_vio_flg == LIO || LidarMeasures.lio_vio_flg == LO)
  {
  ...
  // For VIO mode, accumulate and publish RGB pointcloud
  if (LidarMeasures.lio_vio_flg == VIO)
  {
    *pcl_wait_pub += *pcl_w_wait_pub;
    
    // Memory management: limit accumulated point cloud size
    const size_t MAX_ACCUMULATED_POINTS = 200000; // Optimized for 12GB RAM (was 100k)
    if (pcl_wait_pub->size() > MAX_ACCUMULATED_POINTS)
    {
      ROS_WARN_THROTTLE(1.0, "[Memory] Point cloud buffer too large (%zu), clearing", pcl_wait_pub->size());
    ...
    if(pub_num >= pub_scan_num)
    {
      ...
      // Clear after processing to prevent accumulation
      PointCloudXYZI().swap(*pcl_wait_pub);
    }
  }
  ...
  if (pcd_save_en)
  {
    ...
    // LIVO mode: add RGB point cloud (LIO mode already adds to pcl_wait_save_intensity at line ~1364)
    if (slam_mode_ == LIVO)
    {
      *pcl_wait_save += *laserCloudWorldRGB;
    }
    
    // Memory management: limit pcl_wait_save size to prevent OOM
    const size_t MAX_PCL_WAIT_SAVE_POINTS = 500000; // Limit to ~2GB for RGB point clouds
    if (pcl_wait_save->size() > MAX_PCL_WAIT_SAVE_POINTS)
    {
      ROS_WARN_THROTTLE(1.0, "[Memory] pcl_wait_save too large (%zu points), clearing oldest", pcl_wait_save->size());
      PointCloudXYZRGB().swap(*pcl_wait_save);
    }
    if (pcl_wait_save_intensity->size() > MAX_PCL_WAIT_SAVE_POINTS * 2) // Intensity uses less memory
    {
      ROS_WARN_THROTTLE(1.0, "[Memory] pcl_wait_save_intensity too large (%zu points), clearing oldest", pcl_wait_save_intensity->size());
      PointCloudXYZI().swap(*pcl_wait_save_intensity);
    }
